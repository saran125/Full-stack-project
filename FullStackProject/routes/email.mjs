...
async function sendMail(email, res) {
    try {
        ...
        const mailOptions = {
            from: 'Golden TV',
            to: "kumarsaran522@gmail.com",
            subject: 'Your Room Details',
            text: '',
            html: "<h1>HELLO Saran.</h1><h2>You have suceessfully booked a room</h2>",
        };
        const result = await transport.sendMail(mailOptions);
        console.log('Sent email');
        return res.redirect('/booked_successfully'), result;
    } catch (error) {
        return error;
    }
}
